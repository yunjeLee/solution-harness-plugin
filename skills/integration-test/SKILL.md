...
4. **단언 설계**: 경계마다 "무슨 데이터·계약이 건너가나"를 짚어 단언할 케이스를 목록화한다. (직렬화/매핑/계약/에러 전파 등 경계에서만 깨지는 것)

5. **작성**: TESTING.md 가 정의한 패턴으로 코드를 쓴다.
   - **real code 우선, mock 최소** (tdd 철학 계승). 경계 양쪽을 실제로 통과시킨다.
   - 호스트 자동 레벨(예: Android JVM/in-memory, iOS XCTest)로 작성한다. 기기/시뮬레이터가 필요한 플로우는 이 스킬이 아니라 `e2e-test` 소관.
   - ⚠️ **Robolectric 이 필요하면 `ui` 레벨이므로 이 스킬이 아니다.** `shared/test-levels.md` §1 의 경계 기준은 "Robolectric 없이 JVM 단독으로 도는가"다. Robolectric 을 끌어와야 한다면 `e2e-test` 소관이며, `ui` 가 꺼져 있으면 아예 작성하지 않는다.

6. **실행 안 함**: 작성만 한다. 통과 확인은 7단계 `completion-verifier` 가 전체 스위트와 함께 격리 실행한다. 여기서 직접 돌려 메인 컨텍스트를 오염시키지 않는다.

7. **기록**: 작성한 테스트 파일·대상 경계를 `.harness/runs/run-{id}.md` 에 남긴다. (work 가 "integration-test: 호출" 로그와 함께 추적)
